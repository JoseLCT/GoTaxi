...
public class PIPerfil extends JPanel implements Interfaz {

    private Dimension tamano;
    private Pasajero pasajero;
    private Pasajero pasajero_aux;
    ...
    private JButton btn_cambiar_conductor;
    ...
    public PIPerfil(Dimension tamano) {
        this.tamano = tamano;
    ...
    private void init() {
        ...
        btn_cambiar_conductor = new JButton();
        ...
        btn_cambiar_conductor.setBounds(tamano.width - 55, 15, 40, 40);

        BufferedImage bimg_logo_valoracion = null;
        BufferedImage bimg_cambiar_conductor = null;

        Image img_logo_valoracion;
        Image img_cambiar_conductor;

        ImageIcon icon_logo_valoracion;
        ImageIcon icon_cambiar_conductor;

        try {
            bimg_logo_valoracion = ImageIO.read(new File("Imagenes/valoracion.png"));
            bimg_cambiar_conductor = ImageIO.read(new File("Imagenes/conductor.png"));
        } catch (Exception e) {
        ...
        img_cambiar_conductor = bimg_cambiar_conductor.getScaledInstance(btn_cambiar_conductor.getWidth(), btn_cambiar_conductor.getHeight(), Image.SCALE_SMOOTH);

        icon_logo_valoracion = new ImageIcon(img_logo_valoracion);
        icon_cambiar_conductor = new ImageIcon(img_cambiar_conductor);

        lb_logo_valoracion.setIcon(icon_logo_valoracion);
        btn_cambiar_conductor.setIcon(icon_cambiar_conductor);

        btn_cambiar_conductor.setBorder(null);
        btn_cambiar_conductor.setContentAreaFilled(false);
        ...
        btn_cambiar_conductor.addActionListener(e -> {
            ConductorDao conductorDao = new ConductorDao();
            if (conductorDao.esConductor(pasajero.getUsuario())) {
                conductorDao.iniciarSesion(pasajero.getUsuario());
                notificador.firePropertyChange("piperfil_ciperfil", null, null);
            } else {
                notificador.firePropertyChange("piperfil_ciregistro", null, null);
            }
        });
        ...
        btn_guardar.addActionListener(e -> {
            ...
            pasajero_aux = new Pasajero();
            pasajero_aux.setUsuario(usuario);
            pasajero_aux.setNombre(nombre);
            pasajero_aux.setAppaterno(appaterno);
            pasajero_aux.setApmaterno(apmaterno);
            pasajero_aux.setEmail(email);
            pasajero_aux.setTelefono(telefono);

            try {
                usuarioDao.actualizarDatos(pasajero_aux, pasajero.getUsuario());
        ...
        btn_menu.addActionListener(e -> {
            notificador.firePropertyChange("piperfil_pimenu", null, null);
        });
        btn_historial.addActionListener(e -> {
            notificador.firePropertyChange("piperfil_pihistorial", null, null);
        });
    }
    ...
    public void actualizarDatos() {
        ...
        try {
            lb_valoracion.setText(pasajeroDao.valoracionPromedio(pasajero) + "");
        } catch (Exception e) {
            e.printStackTrace();
        }

        txt_usuario.setText(pasajero.getUsuario());
        txt_nombre.setText(pasajero.getNombre());
        txt_appaterno.setText(pasajero.getAppaterno());
        txt_apmaterno.setText(pasajero.getApmaterno());
        txt_ci.setText(pasajero.getCi());
        txt_email.setText(pasajero.getEmail());
        txt_telefono.setText(pasajero.getTelefono());
        fecha.setDate(pasajero.getFecha_nacimiento());
    }
